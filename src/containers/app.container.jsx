...
const binary = window.require('@ffmpeg-installer/ffmpeg');
const util = window.require('electron-util');
const ffmpegPath = util.fixPathForAsarUnpack(binary.path);
...
class AppContainer extends Component {
  ...
  getVideoAsMp4(urlLink, userProvidedPath, title) {
    ...
    return new Promise((resolve, reject) => {
      let fullPath = path.join(userProvidedPath, `${title}`);
  ...
  async startDownload(id) {
    ...
    try {
      ...
      let info = await ytdl.getInfo(id, {lang: 'zh-TW'});
      console.log(await ytdl.getInfo(id, {lang: 'zh_TW'}));
      console.log(info);
      let title = info.player_response.videoDetails.title || info.player_response.videoDetails.author+"的歌";
      ...
      let paths = await this.getVideoAsMp4(id, this.state.userDownloadsFolder, title);
